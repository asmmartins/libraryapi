...
namespace Library.Tests.Unit.Domain.Books
{
    public class BooksTests
    {
        ...
        {
            ValidationException ex = Assert.Throws<ValidationException>(() => BookDomain.Book.Create(title, null, 1, null, 00.00m));
        ...
        {
            ValidationException ex = Assert.Throws<ValidationException>(() => BookDomain.Book.Create("Livro 1", publishingCompany, 0, null, 00.00m));
            ex.AssertErrorMessage(errorMessage);
        }
        ...
        {
            ValidationException ex = Assert.Throws<ValidationException>(() => BookDomain.Book.Create("Livro 1", "Casa do Saber", edition, null, 00.00m));
            ex.AssertErrorMessage(errorMessage);
        }
        ...
        {
            ValidationException ex = Assert.Throws<ValidationException>(() => BookDomain.Book.Create("Livro 1", "Casa do Saber", 1, publicationYear, 00.00m));
            ex.AssertErrorMessage(errorMessage);
        }
        ...
        {
            var book = BookDomain.Book.Create(title, publishingCompany, edition, publicationYear, price);
